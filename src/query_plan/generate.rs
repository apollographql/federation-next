...
pub trait GenerateContext<Plan, Element> {
    /// `add`: how to obtain a new plan by taking some plan and adding a new element to it.
    fn add(&mut self, plan: &Plan, elem: Element) -> Plan;

    /// `cost`: how to compute the cost of a plan.
    fn cost(&mut self, plan: &mut Plan) -> Result<QueryPlanCost, FederationError>;
...
/// * `context`: a struct that implements the `GenerateContext` trait.
pub fn generate_all_plans_and_find_best<Plan, Element>(
    mut initial: Plan,
    to_add: Vec<Choices<Element>>,
    mut context: impl GenerateContext<Plan, Element>,
...
{
    if to_add.is_empty() {
        let cost = context.cost(&mut initial)?;
    ...
    }) = stack.pop_back()
    {
        ...
        let mut new_partial_plan = context.add(&partial_plan, extracted);
        let cost = context.cost(&mut new_partial_plan)?;
        ...
        if remaining.as_slice().is_empty() {
            ...
            context.on_plan(&new_partial_plan, cost, previous_min_cost);
...
mod tests {
    ...
    struct TestGenerateContext<'a> {
        generated: &'a mut Vec<Vec<&'static str>>,
        target_len: usize,
    }

    impl<'a> GenerateContext<Plan, Element> for TestGenerateContext<'a> {
        fn add(&mut self, partial_plan: &Plan, new_element: Element) -> Plan {
    ...
    fn generate_test_plans(initial: Plan, choices: Vec<Vec<Option<Element>>>) -> (Plan, Vec<Plan>) {
        ...
        let context = TestGenerateContext {
            generated: &mut generated,
            target_len,
        };
        let (best, _) =
            generate_all_plans_and_find_best::<Plan, Element>(initial, choices, context).unwrap();
        (best, generated)
    }
