...
fn graphql_value_to_json(
    ...
    match value {
        ...
        Value::Float(value) => {
            if !value.is_finite() {
                // TODO: ensure validation does checks this
                validation_should_have_caught_this!(
                    "Non-finite float in default value of {kind} {parent}{sep}{name}."
                )
            }
            Ok(value.0.into())
        }
        Value::BigInt(value) => {
            ...
            // TODO: ensure validation does checks this
            validation_should_have_caught_this!(
                "Invalid BigInt in default value of  {kind} {parent}{sep}{name}."
            )
        }
...
pub(crate) fn coerce_argument_values(
    ...
    for arg_def in &field_def.arguments {
        ...
        if let Some((_name, value)) = field
            .arguments
            .iter()
            .find(|(name, _value)| name == arg_name)
        {
            if let Value::Variable(var_name) = value.as_ref() {
