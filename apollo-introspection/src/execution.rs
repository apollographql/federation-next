...
pub(crate) async fn execute_selection_set<'a>(
    ...
    let futures = grouped_field_set
        ...
        .filter_map(|(&response_key, fields)| {
            ...
            let Some(field_def) = schema.type_field(object_type_name, field_name) else {
...
fn does_fragment_type_apply(
    ...
    match schema.types.get(fragment_type) {
        ...
        Some(ExtendedType::Interface(_)) => object_type
            ...
            .contains_key(fragment_type),
        Some(ExtendedType::Union(def)) => def.members.contains_key(object_type_name),
