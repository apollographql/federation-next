...
impl SchemaIntrospectionQuery {
    ...
    /// [schema introspection]: https://spec.graphql.org/October2021/#sec-Schema-Introspection
    pub fn split_from(
        document: &mut Arc<ExecutableDocument>,
        ...
        let document = document.make_mut();
        ...
        let operation = Operation {
            ...
            directives: Vec::new(), // unused by execution
            selection_set: SelectionSet { ty, selections },
        };
        let mut fragments: IndexMap<_, _> = fragments_to_split
            .into_iter()
            .map(|name| {
                ...
                (name, Node::new_synthetic(new_fragment))
            })
            .collect();
        let intropsection_fragments: Vec<_> = document
            ...
            .collect();
        fragments.extend(intropsection_fragments.into_iter().map(|name| {
            let def = document.fragments.remove(&name).unwrap();
            (name, def)
        }));
        Ok(Self {
            document: Some(ExecutableDocument {
                named_operations: Default::default(),
                anonymous_operation: Some(Node::new_synthetic(operation)),
                fragments,
            }),
        })
    }
        ...
        match selection {
            ...
            Selection::InlineFragment(inline) => {
                ...
                if !collected.is_empty() {
                    extracted.push(Selection::InlineFragment(Node::new_synthetic(
                        InlineFragment {
...
impl_resolver! {
    ...
    async fn queryType(&self_) {
        Ok(type_def_opt(*self_, &self_.query_type))
    }

    async fn mutationType(&self_) {
        Ok(type_def_opt(*self_, &self_.mutation_type))
    }

    async fn subscriptionType(&self_) {
        Ok(type_def_opt(*self_, &self_.subscription_type))
    }
}
...
impl_resolver! {
    ...
    async fn fields(&self_, args) {
        ...
        Ok(ResolvedValue::list(fields
            .values()
            .filter(move |def| {
                include_deprecated || def.directive_by_name("deprecated").is_none()
            })
            .map(|def| {
    ...
    async fn interfaces(&self_) {
        ...
        Ok(ResolvedValue::list(implements_interfaces.keys().filter_map(|name| {
            self_.schema.types.get(name).map(|def| {
    ...
    async fn possibleTypes(&self_) {
        ...
        match self_.def {
            ...
            schema::ExtendedType::Union(def) => types!(def.members.keys()),
...
impl_resolver! {
    ...
    async fn isDeprecated(&self_) {
        Ok(ResolvedValue::leaf(self_.def.directive_by_name("deprecated").is_some()))
    }

    async fn deprecationReason(&self_) {
        Ok(deprecation_reason(self_.def.directive_by_name("deprecated")))
    }
}
